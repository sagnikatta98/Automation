...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        gyro_columns = ['Gyroscope (dps).x', 'Gyroscope (dps).y', 'Gyroscope (dps).z']
        gyro_raw_columns = ['Gyro Raw 0.x', 'Gyro Raw 0.y', 'Gyro Raw 0.z']
        converted_columns = [col.replace('Gyroscope (dps)', 'Gyroscope (in rps)') for col in gyro_columns]
        for col, conv_col in zip(gyro_columns, converted_columns):
            df[conv_col] = pd.to_numeric(df[col], errors='coerce') * (np.pi/180)
        avg_columns = [col.replace('Gyroscope (dps)', 'Gyro Avg') for col in gyro_columns]
        for conv_col, avg_col in zip(converted_columns, avg_columns):
            df[avg_col] = df[conv_col].rolling(window=8, min_periods=8).mean()
        for raw_col in gyro_raw_columns:
            df[raw_col] = pd.to_numeric(df[raw_col], errors='coerce')
        diff_columns = [col.replace('Gyro Raw 0', 'Gyro Diff') for col in gyro_raw_columns]
        for avg_col, raw_col, diff_col in zip(avg_columns, gyro_raw_columns, diff_columns):
            df[diff_col] = df[avg_col] - df[raw_col]
        for diff_col in diff_columns:
            df[diff_col] = df[diff_col].round(5)
        df_after_50 = df[diff_columns].iloc[50:]
        print("\nValues after 50 samples (Gyro Diff columns):")
        print(df_after_50.head(20))
        df_after_50_normalized = df_after_50.applymap(lambda x: 0 if abs(x) < 1e-5 else x)
        is_valid = df_after_50_normalized.isin([0, float('nan')]).all().all()
        if is_valid:
            print("PASS")
        else:
            print("FAIL")
            invalid_rows = df_after_50_normalized[~df_after_50_normalized.isin([0, float('nan')]).all(axis=1)]
            print("Indexes and values of nonzero entries:")
            print(invalid_rows)
        processed_df = df[gyro_columns + converted_columns + avg_columns + gyro_raw_columns + diff_columns]
