...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        start_label = "stable_point"
        df.columns = df.columns.str.strip()
        stable_idx = df[df['Label data .l'].str.contains(start_label, na=False)]
        if stable_idx.empty:
            print("No 'stable_point' found in the dataset.")
            return df
        stable_idx = stable_idx.index[0]
        df.loc[stable_idx:, 'Game Rotation vector 0.x'] = df.loc[stable_idx:, 'Game Rotation vector 0.x'].replace("", pd.NA).dropna()
        df['Time Difference'] = df['Time(s.ns)'].diff()
        settling_times = df.loc[stable_idx + 1:, 'Time Difference'].dropna()
        avg_settling_time = settling_times.mean()
        print(f"Average settling time: {avg_settling_time:.4f} seconds")
        if avg_settling_time < 0.2:
            print("PASS: Settling time is within expected range.")
        else:
            print("FAIL: Settling time is above expectation.")
        return df
