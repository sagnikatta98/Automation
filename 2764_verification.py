...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        """ Validate the contents of the CSV file by loading it into a DataFrame, converting values to degrees, and plotting orientation data """
        print(f"Reading {csv_file} into a dataframe...")
        df = pd.read_csv(csv_file)
        df.columns = df.columns.str.strip()
        if 'Linear Accel 0.a' in df.columns:
            df['Linear Accel 0.a'] = pd.to_numeric(df['Linear Accel 0.a'], errors='coerce') 
            index_start = df[df['Linear Accel 0.a'] >= 3].index.min()
            if not pd.isna(index_start):
                accel_cols = ['Linear Accel 0.x', 'Linear Accel 0.y', 'Linear Accel 0.z','Gravity 0.x', 'Gravity 0.y', 'Gravity 0.z',
                            'Accel Corrected 0.x', 'Accel Corrected 0.y', 'Accel Corrected 0.z']
                df[accel_cols] = df[accel_cols].apply(pd.to_numeric, errors='coerce')
                df.loc[index_start:, 'Difference X'] = (df['Linear Accel 0.x'] + df['Gravity 0.x'] - df['Accel Corrected 0.x']).round(5)
                df.loc[index_start:, 'Difference Y'] = (df['Linear Accel 0.y'] + df['Gravity 0.y'] - df['Accel Corrected 0.y']).round(5)
                df.loc[index_start:, 'Difference Z'] = (df['Linear Accel 0.z'] + df['Gravity 0.z'] - df['Accel Corrected 0.z']).round(5)
                print(df.loc[index_start:, ['Difference X', 'Difference Y', 'Difference Z']].head())
                print("Applied correction from index:", index_start)
                invalid_indices = df.loc[index_start:, ['Difference X', 'Difference Y', 'Difference Z']].dropna().ne(0).any(axis=1)
                if invalid_indices.any():
                    print("FAIL - Nonzero values found at indices:", df.loc[invalid_indices].index.tolist())
                else:
                    print("PASS - All values are either 0 or blank.")
                updated_csv_file = csv_file.replace(".csv", "_verified.csv")
                df.to_csv(updated_csv_file, index=False)
                print(f"Updated CSV saved: {updated_csv_file}")
        return df
