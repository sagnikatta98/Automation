...
class BLEConfigurator:
    ...
    async def run(self):
        ...
        try:
            ...
            await self.send_command("imux ed 1")
            await self.send_command("-l 1 4679.bin")
            await self.send_command("actse 72 50")
            await self.send_command("actse 9 200")
            print("Gyro calibration started. Keep device stable for 15 seconds")
            while not self.gyro_calibration_complete:
                await asyncio.sleep(1)
            print("Perform Accel calibration by keeping each side of device stable for 3-4 secs...")
