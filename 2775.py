...
class BLEConfigurator:
    ...
    async def nus_data_rcv_handler(self, sender, data):
        ...
        if "Gyro Accuracy" in decoded_data:
            ...
            if accuracy == 3 and self.gyro_calibration_start_time is not None and not self.gyro_calibration_complete:
    ...
    async def run(self):
        ...
        try:
            ...
            await self.send_command("imux fc 1")
            await self.send_command("-l 1 2777555.bin")
            await self.send_command("actse 54 100")
            await self.send_command("actse 52 100")
            print("Accel & Gyroscope calibration started...")
            ...
            while not self.accel_calibration_complete:
                await asyncio.sleep(1)
            print("Take the device in hand and perform random movements for 30secs")
