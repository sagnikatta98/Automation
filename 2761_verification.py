...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        orientation_columns = ['Orientation 0 (rad).h', 'Orientation 0 (rad).p', 'Orientation 0 (rad).r']
        for col in orientation_columns:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
                df[col] = df[col] * (180 / math.pi)
                filtered_df = df[(df[col].abs() < 1E+3)]
                if not filtered_df.empty:
                    print(f"Max and Min values for {col}: {filtered_df[col].max():.4f}, {filtered_df[col].min():.4f}")
                    print(f"Generating HTML plot for {col}...")
                    fig = go.Figure()
                    ...
                    fig.update_layout(
                        title=f"2D Plot of {col}",
                        xaxis_title="Index",
                        yaxis_title=col,
                        template="plotly_white")
                    if col == 'Orientation 0 (rad).h':
                        annotation_text = "PASS"
                    else:
                        contains_negative = (filtered_df[col] < 0).any()
                        annotation_text = "PASS" if contains_negative else "FAIL"
                    fig.add_annotation(
                    ...
                    html_file = f"{col.replace(' ', '_').replace('(rad)', '').lower()}_plot_2761.html"
                    fig.write_html(html_file)
                    print(f"Plot saved as {html_file}.")
                else:
                    print(f"All values for {col} were filtered out due to being too large.")
            else:
