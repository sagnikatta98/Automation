...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        col_a = 'IMUX Calib 0.a'
        col_g = 'IMUX Calib 0.g'
        if col_a not in df.columns or col_g not in df.columns:
        ...
        col_a_unique = df[col_a].dropna().unique().tolist()
        col_g_unique = df[col_g].dropna().unique().tolist()
        col_a_all_zeros = set(col_a_unique) == {0} or len(col_a_unique) == 0
        col_g_all_zeros = set(col_g_unique) == {0} or len(col_g_unique) == 0
        col_a_has_all = set(col_a_unique) == {0, 1, 2, 3}
        col_g_has_all = set(col_g_unique) == {0, 1, 2, 3}
        zero_column = col_a if col_a_all_zeros else (col_g if col_g_all_zeros else None)
        all_values_column = col_a if col_a_has_all else (col_g if col_g_has_all else None) 
        if col_a_all_zeros and col_g_all_zeros:
            print("PASS: Both columns have only zeros")
        elif col_a_has_all and col_g_has_all:
            print("PASS: Both columns have values 0,1,2,3")
        elif zero_column and all_values_column:
            print(f"PASS: {zero_column} has only zeros, {all_values_column} has values 0,1,2,3")
        else:
            print("FAIL")
        return df
