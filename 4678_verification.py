...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        accel_pass_columns = ['Accel Passthrough 0.x', 'Accel Passthrough 0.y', 'Accel Passthrough 0.z']
        ...
        for conv_col, avg_col in zip(converted_columns, avg_columns):
            df[avg_col] = df[conv_col].rolling(window=2, min_periods=2).mean()
        for pass_col in accel_pass_columns:
            df[pass_col] = pd.to_numeric(df[pass_col], errors='coerce')
        diff_columns = [col.replace(' Accel Passthrough 0', 'Accel Diff') for col in accel_pass_columns]
        for avg_col, pass_col, diff_col in zip(avg_columns, accel_pass_columns, diff_columns):
            df[diff_col] = df[avg_col] - df[pass_col]
        for diff_col in diff_columns:
            df[diff_col] = df[diff_col].round(5)
        df_after_116 = df[diff_columns].iloc[116:]
        print("\nValues after 115 samples (Accel Diff columns):")
        print(df_after_116.head(116))
        df_after_116_normalized = df_after_116.applymap(lambda x: 0 if abs(x) < 1e-5 else x)
        is_valid = df_after_116_normalized.isin([0, float('nan')]).all().all()
        if is_valid:
            print("PASS")
        else:
            print("FAIL")
            invalid_rows = df_after_116_normalized[~df_after_116_normalized.isin([0, float('nan')]).all(axis=1)]
            print("Indexes and values of nonzero entries:")
            print(invalid_rows)
        processed_df = df[accel_columns + converted_columns + avg_columns + accel_pass_columns + diff_columns]
