...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        for column_name in columns:
            html_content = "<html><body><h1>Result</h1>"
            if column_name in df.columns:
                df[column_name] = pd.to_numeric(df[column_name], errors="coerce")
                invalid_values = df[(df[column_name].notna()) & (df[column_name] != 3)]
                if not invalid_values.empty:
                    print("FAIL")
                    html_content += f"<p style='color:red;'>Error: Column '{column_name}' contains values other than 3.</p>"
                    print(f"FAIL: Column '{column_name}' contains values other than 3:")
                    print(invalid_values[[column_name]])
                else:
                    html_content += f"<p style='color:green;'>Pass: Column '{column_name}' is valid (only contains 3 or blanks).</p>"
                    print(f"PASS: Column '{column_name}' is valid (only contains 3 or blanks).")
                ...
                html_content += f"<p style='color:orange;'>Column '{column_name}' not found in the CSV file.</p>"
                print(f"Column '{column_name}' not found in the CSV file.")
            html_content += "</body></html>"
            html_filename = f"2760_{column_name.replace(' ', '_').replace('.', '')}.html"
