...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        gyro_pass_columns = ['Gyro Passthrough 0.x', 'Gyro Passthrough 0.y', 'Gyro Passthrough 0.z']
        ...
        for raw_col in gyro_pass_columns:
            df[raw_col] = pd.to_numeric(df[raw_col], errors='coerce')
        diff_columns = [col.replace('Gyro Raw 0', 'Gyro Diff') for col in gyro_pass_columns]
        for avg_col, raw_col, diff_col in zip(avg_columns, gyro_pass_columns, diff_columns):
        ...
        if is_valid:
            ...
            print(invalid_rows)
        processed_df = df[gyro_columns + converted_columns + avg_columns + gyro_pass_columns + diff_columns]
