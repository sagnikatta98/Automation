...
class BLEFileHandler:
    ...
    def validate_csv(self, csv_file):
        ...
        accel_columns = ["Accel Corrected 0.x", "Accel Corrected 0.y", "Accel Corrected 0.z"]
        for col in accel_columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")
        df_filtered = df[df["Accel Corrected 0.a"] == 3]
        if not df_filtered.empty:
            print("Generating HTML plot for Accel Corrected values...")
            fig = go.Figure()
            colors = ['red', 'green', 'blue']
            for col, color in zip(accel_columns, colors):
            ...
            fig.update_layout(
            title="2D Plot of Accel Corrected Values (After Accel Accuracy Reaches 3)",
            xaxis_title="Index",
            yaxis_title="Accel Corrected Values",
            template="plotly_white")
            fig.write_html("accel_corrected_plot_2752.html")
            print("Plot saved as accel_corrected_plot_2752.html")
        else:
            print("No data found where Accel Accuracy is 3.")
        return df
